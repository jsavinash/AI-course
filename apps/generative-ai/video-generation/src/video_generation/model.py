...
@dataclass
class SpatiotemporalDiffusionModel:
    img_size: int = 32
    n_frames: int = 8
    latent_dim: int = 64
...
class VideoGenerationModel:
    ...
    encoder: LatentVideoEncoder | None = None
    diffusion: SpatiotemporalDiffusionModel | None = None
    loss_history: list[float] = field(default_factory=list, repr=False)
    _cache: dict = field(default_factory=dict, repr=False)

    def _init(self) -> None:
        self.tokenizer = VideoTokenizer(vocab_size=self.vocab_size, random_seed=self.random_seed)
        ...
        self.encoder = LatentVideoEncoder(img_size=self.img_size, n_frames=self.n_frames, latent_dim=self.latent_dim, random_seed=self.random_seed + 2)
        self.diffusion = SpatiotemporalDiffusionModel(img_size=self.img_size, n_frames=self.n_frames, latent_dim=self.latent_dim, n_diffusion_steps=self.n_diffusion_steps, random_seed=self.random_seed + 3)
    ...
    def encode_video(self, video: np.ndarray) -> np.ndarray:
        if self.encoder is None:
            self._init()
        flat = video.reshape(1, -1)
        mu, logvar = self.encoder.encode(flat)
        return self.encoder.reparameterize(mu, logvar)

    def decode_latent(self, z: np.ndarray) -> np.ndarray:
        if self.encoder is None:
            self._init()
        reconstructed = self.encoder.decode(z)
        return reconstructed.reshape(self.n_frames, self.img_size, self.img_size, 3)
    ...
    def animate_from_image(self, image: np.ndarray, motion_prompt: str, n_steps: int = 50) -> np.ndarray:
        if self.diffusion is None or self.encoder is None:
            self._init()
        image_flat = image.reshape(1, -1)
        mu, _ = self.encoder.encode(image_flat)
        z_T = np.random.default_rng(self.random_seed).normal(0, 1, size=(1, self.latent_dim))
        z_T = 0.5 * z_T + 0.5 * mu
        text_cond = self.encode_text(motion_prompt)
        z_0 = self.diffusion.reverse_process(z_T, text_cond=text_cond, n_steps=n_steps)
        return self.decode_latent(z_0)

    def fit(self, X_videos: np.ndarray, X_texts: np.ndarray, n_iterations: int | None = None) -> dict:
        if self.encoder is None or self.diffusion is None:
            self._init()
        if n_iterations is None:
            n_iterations = self.n_iterations
        n_samples = X_videos.shape[0]
        rng = np.random.default_rng(self.random_seed)
        total_loss = 0.0
        for _epoch in range(n_iterations):
            perm = rng.permutation(n_samples)
            X_vid_shuffled = X_videos[perm]
            epoch_loss = 0.0
            for i in range(n_samples):
                video = X_vid_shuffled[i].reshape(1, -1)
                mu, logvar = self.encoder.encode(video)
                z = self.encoder.reparameterize(mu, logvar)
                reconstructed = self.encoder.decode(z)
                recon_loss = float(np.mean((reconstructed - video) ** 2))
    ...
    def evaluate(self, X_videos: np.ndarray, X_texts: np.ndarray) -> dict[str, float]:
        n_samples = X_videos.shape[0]

        total_recon = 0.0
        for i in range(n_samples):
            video = X_videos[i].reshape(1, -1)
            mu, _ = self.encoder.encode(video)
            z = mu
            reconstructed = self.encoder.decode(z)
            total_recon += float(np.mean((reconstructed - video) ** 2))
    ...
    def save(self, path: str) -> None:
        arrays = {
            ...
            "n_frames": np.array([self.n_frames]),
        ...
        if self.encoder and self.encoder.encoder_w is not None:
            arrays["encoder_w"] = self.encoder.encoder_w
            arrays["encoder_b"] = self.encoder.encoder_b
            arrays["mu_w"] = self.encoder.mu_w
            arrays["mu_b"] = self.encoder.mu_b
            arrays["logvar_w"] = self.encoder.logvar_w
            arrays["logvar_b"] = self.encoder.logvar_b
            arrays["decoder_w"] = self.encoder.decoder_w
            arrays["decoder_b"] = self.encoder.decoder_b
    ...
    @classmethod
    def load(cls, path: str) -> "VideoGenerationModel":
        ...
        obj._init()
        if "encoder_w" in data and obj.encoder:
            obj.encoder.encoder_w = data["encoder_w"]
            obj.encoder.encoder_b = data["encoder_b"]
            obj.encoder.mu_w = data["mu_w"]
            obj.encoder.mu_b = data["mu_b"]
            obj.encoder.logvar_w = data["logvar_w"]
            obj.encoder.logvar_b = data["logvar_b"]
            obj.encoder.decoder_w = data["decoder_w"]
            obj.encoder.decoder_b = data["decoder_b"]
