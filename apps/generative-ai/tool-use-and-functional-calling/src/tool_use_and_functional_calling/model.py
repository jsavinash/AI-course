...
class ToolUseModel:
    ...
    def _register_default_tools(self) -> None:
        for tool_name, tool_data in PREDEFINED_TOOLS.items():
            self.register_tool(
                ToolSpec(
                    name=tool_data["name"],
...
def _synthesize_response(query: str, tool_name: str, tool_output: str, success: bool) -> str:
    ...
    if tool_name == "get_stock_price":
        return f"Based on the tool output, {tool_output}."

    elif tool_name == "get_weather":
        return f"Based on the current weather data: {tool_output}."

    elif tool_name == "calculator":
        return f"Based on the calculation: {tool_output}."

    elif tool_name == "get_order_status":
        return f"Here is your order information: {tool_output}."

    elif tool_name == "execute_sql_query":
        return f"Here are the query results: {tool_output}."

    return f"Tool '{tool_name}' returned: {tool_output}."
