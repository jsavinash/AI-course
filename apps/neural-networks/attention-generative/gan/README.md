...
This example is grounded in **Generative Adversarial Network (GAN)**. The equations below
...
```
"""Data loading and preprocessing for GAN-based image generation.
...
def generate_latent(n_samples: int, latent_dim: int = LATENT_DIM, random_seed: int = 42) -> np.ndarray:
    ...
    rng = np.random.default_rng(random_seed)
    return rng.normal(0, 1, size=(n_samples, latent_dim))
```
