...
![Machine Learning Fundamentals diagram](./assets/advanced-generative-art.png)
...
| `GenerativeArtDN` | _build, fit, predict, predict_proba, mse, rmse, evaluate, save, load, to_dict | Deconvolutional network for generative art.  Args:     img_size: Size of input images (square)     n_channels: Number of input/output channels     n_filters: Number of filters in conv layers     kernel_size: Convolution kernel size     learning_rate: Gradient descent step size     n_iterations: Number of training epochs     weight_decay: L2 regularization strength     random_seed: Random seed |
...
### `GenerativeArtDN.fit(X, y, X_val, y_val)`
...
```
"""Generative Art using a Deconvolutional Network (DN).
...
from advanced_generative_art.data import reshape_image
...
@dataclass
class GenerativeArtDN:
    """Deconvolutional network for generative art.
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, X_val=None, y_val=None) -> "GenerativeArtDN":
        """Train the deconvolutional network.
    ...
    @classmethod
    def load(cls, path: str) -> "GenerativeArtDN":
...
```
"""Training pipeline for Generative Art (DN)."""
...
```
"""Data loading and preprocessing for Generative Art (DN).
...
```
"""Serving API for Generative Art (DN)."""
