...
$$v_j = \frac{\|s_j\|^2}{1 + \|s_j\|^2} \frac{s_j}{\|s_j\|}, \quad \text{squash function}$$

$$\mathcal{L}_k = T_k \max(0, m^+ - \|v_k\|)^2 + \lambda (1 - T_k) \max(0, \|v_k\| - m^-)^2$$
...
### `TextCharRecognitionCapsNet.fit(X, y, X_val, y_val)`
...
### `TextCharRecognitionCapsNet.predict(X)`
...
```
"""Text and Character Recognition using a Capsule Network.

Architecture:
    Input (1 x 8x8) -> Conv2D (8, 3x3, ReLU) -> ConvCapsule (36, 8) -> Dense (36, softmax)
...
from capsnet_text_recognition.data import reshape_image

@dataclass
class TextCharRecognitionCapsNet:
    """Capsule Network for text and character recognition.
    ...
    @classmethod
    def load(cls, path: str) -> "TextCharRecognitionCapsNet":
...
```
"""Training pipeline for Text and Character Recognition (CapsNet)."""
...
```
"""Data loading and preprocessing for Text and Character Recognition (CapsNet).
...
```
"""Serving API for Text and Character Recognition (CapsNet)."""
