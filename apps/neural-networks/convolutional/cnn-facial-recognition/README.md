...
### `FacialRecognitionCNN.fit(X, y, X_val, y_val)`
...
### `FacialRecognitionCNN.predict(X, threshold)`
...
Architecture:
    ...
    -> MaxPool2D (2x2) -> Flatten -> Dense (32, ReLU) -> Dense (1, sigmoid)

Loss: binary cross-entropy (many-to-one: classifies image into a class label)
...
from cnn_facial_recognition.data import reshape_image
...
class FacialRecognitionCNN:
    ...
    @classmethod
    def load(cls, path: str) -> "FacialRecognitionCNN":
...
```
"""Training pipeline for Facial Recognition (CNN)."""
...
from ai_core.validation import DataValidator, create_facial_recognition_schema

from cnn_facial_recognition.data import (
    load_training_data,
    save_training_data,
    train_test_split,
)
...
def train(
    ...
    """Train the facial recognition model and save artifacts."""
    ...
    model_path = model_dir / f"facial_recognition_model_v{model_version}.npz"
    ...
    registry.save_model(
        ...
        artifacts={
            f"facial_recognition_model_v{model_version}.npz": model_path,
            "training_data.npz": model_dir / "training_data.npz",
        },
        tags={"framework": "numpy", "task": "facial_recognition", "model_type": "CNN"},
    )
    ...
    if register_to_mlflow:
        registry.log_to_mlflow(
            ...
            artifacts={
                ...
                "chart": str(model_dir / f"facial_recognition_v{model_version}.png"),
            },
            tags={"model_type": "facial_recognition", "framework": "numpy"},
        )
        logger.info("Registered model to MLflow", model="facial-recognition", version=model_version)
...
def _save_chart(model, output_dir: Path, version: str) -> None:
    ...
    ax.set_ylabel("Loss")
    ax.set_title("Facial Recognition CNN Training Loss")
    ax.grid(True, alpha=0.3)
    ax.set_yscale("log")
    plt.tight_layout()
    chart_path = output_dir / f"facial_recognition_v{version}.png"
...
```
"""Data loading and preprocessing for Facial Recognition (CNN).
...
```
"""Serving API for Facial Recognition (CNN)."""
...
from cnn_facial_recognition.data import IMAGE_SIZE, N_CHANNELS, generate_synthetic_data
from cnn_facial_recognition.model import FacialRecognitionCNN
...
METRICS_PORT = int(os.getenv("FACIAL_RECOGNITION_METRICS_PORT", "8017"))
DRIFT_THRESHOLD = float(os.getenv("DRIFT_THRESHOLD", "0.2"))

class PredictRequest(BaseModel):
    is_owner: list[float] = Field(..., min_length=64, max_length=64)

class PredictBulkRequest(BaseModel):
    requests: list[list[float]] = Field(..., min_length=1, max_length=50)

class PredictResponse(BaseModel):
    is_owner: str | list[float] | bool
    confidence: float
    model_version: str
    training_mode: str
...
_model: FacialRecognitionCNN | None = None
...
async def lifespan(app: FastAPI):
    ...
    _metrics = MetricsCollector("facial_recognition", port=METRICS_PORT)
    app.state.metrics = _metrics

    _validator = DataValidator(create_facial_recognition_schema())
    ...
    logger.info("Model loaded", model="facial-recognition", version=_model_version)

    yield
    logger.info("Shutting down facial-recognition API")

def _load_model() -> tuple[FacialRecognitionCNN, str]:
    registry = ModelRegistry(base_dir=MODEL_DIR)
    try:
        if MODEL_VERSION == "latest":
            models = registry.list_models()
            nn_models = [m for m in models if m.get("model_name") == "facial-recognition"]
            if nn_models:
                nn_models.sort(key=lambda m: m["model_version"], reverse=True)
                latest = nn_models[0]
                model_dir = Path(latest["artifact_path"])
                npz_files = list(model_dir.glob("facial_recognition_model_*.npz")) + list(model_dir.glob("*.npz"))
                if npz_files:
                    return FacialRecognitionCNN.load(str(npz_files[0])), latest["model_version"]
        else:
            model_dir = MODEL_DIR / "facial-recognition" / MODEL_VERSION
            if model_dir.exists():
                npz_files = list(model_dir.glob("facial_recognition_model_*.npz")) + list(model_dir.glob("*.npz"))
                if npz_files:
                    return FacialRecognitionCNN.load(str(npz_files[0])), MODEL_VERSION
    except Exception as e:
        logger.warning(f"Registry lookup failed: {e}")

    npz_path = MODEL_DIR / "facial_recognition_model.npz"
    if npz_path.exists():
        return FacialRecognitionCNN.load(str(npz_path)), "legacy"

    candidate_paths = [
        Path("/app/artifacts/models/facial_recognition_model_v1.0.0.npz"),
        Path(__file__).resolve().parents[3] / "artifacts" / "models" / "facial_recognition_model_v1.0.0.npz",
    ]
    for p in candidate_paths:
        if p.exists():
            logger.info("Loading bundled baseline model", path=str(p))
            return FacialRecognitionCNN.load(str(p)), "1.0.0-bundled"
...
app = FastAPI(
    ...
    description="Binary facial recognition for phone unlock and photo tagging",
    version="1.0.0",
    lifespan=lifespan,
)
...
def read_root():
    return {
        "service": "facial_recognition-api",
        "version": "1.0.0",
        "model_version": _model_version,
...
def reload_model():
    ...
    try:
        ...
        _reference_data = _load_reference_data()
        logger.info("Model reloaded dynamically", model="facial-recognition", version=_model_version)
...
def _compute_prediction(is_owner: list[float]):
    if _model is None or _metrics is None or _validator is None:
        raise HTTPException(status_code=503, detail="Model not loaded")

    X = np.array([is_owner]).reshape(1, -1)
    ...
    try:
        ...
        _recent_predictions.append(is_owner)
...
def predict(body: PredictRequest):
    """Make a facial recognition prediction."""
    return _compute_prediction(body.is_owner)

@app.post("/predict/bulk", response_model=BulkPredictResponse)
def predict_bulk(body: PredictBulkRequest):
    """Make multiple facial recognition predictions."""
    ...
    predictions = []
    for is_owner in body.requests:
        predictions.append(_compute_prediction(is_owner))
