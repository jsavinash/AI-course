...
METRICS_PORT = int(os.getenv("METRICS_PORT", os.getenv("SELF_SUPERVISED_MONITORING_METRICS_PORT", "8007")))
...
def _load_model() -> tuple[DenoisingAutoencoder, str]:
    ...
    try:
        if MODEL_VERSION == "latest":
            ...
            if ss_models:
                ...
                npz_files = (
                    list(model_dir.glob("self_supervised_monitoring_model_*.npz"))
    ...
    candidate_paths = [
        ...
        Path(__file__).resolve().parents[3] / "artifacts" / "models" / "self_supervised_monitoring_model_v1.0.0.npz",
    ]
    ...
    logger.warning("No pre-existing model found on disk. Initializing baseline self-supervised model.")
...
def reload_model():
    ...
    try:
        ...
        _reference_data = _load_reference_data()
        logger.info("Model reloaded dynamically", model="self-supervised-monitoring", version=_model_version)
