...
class Apriori:
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if list1[:-1] == list2[:-1]:
                    ...
                    if len(new_itemset) == k and self._has_frequent_subsets(new_itemset, frequent_itemsets):
    ...
    def recommend(
        ...
        sorted_recs = sorted(
            recommendations.values(), key=lambda r: r["score"], reverse=True
        )
    ...
    def save(self, path: str) -> None:
        ...
        np.savez(
            ...
            # Serialize rules
            rules_antecedent=np.array(
                [sorted(r.antecedent) for r in self.rules], dtype=object
            ),
            rules_consequent=np.array(
                [sorted(r.consequent) for r in self.rules], dtype=object
            ),
