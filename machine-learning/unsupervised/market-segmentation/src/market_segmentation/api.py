...
class SegmentRequest(BaseModel):
    ...
    annual_income: float = Field(..., gt=0, le=200, description="Annual income in thousands of dollars")
...
async def lifespan(app: FastAPI):
    ...
    _metrics.set_model_info(model_name="market-segmentation", model_version=_model_version, model_type="clustering")
...
def _load_model() -> tuple[KMeans, str]:
    ...
    try:
        if MODEL_VERSION == "latest":
            ...
            if seg_models:
                ...
                npz_files = list(model_dir.glob("market_segmentation_model_*.npz")) + list(model_dir.glob("*.npz"))
                if npz_files:
    ...
    candidate_paths = [
        ...
        Path(__file__).resolve().parents[3] / "artifacts" / "models" / "market_segmentation_model_v1.0.0.npz",
    ]
...
def reload_model():
    ...
    try:
        ...
        if _metrics:
            ...
            _metrics.set_model_info(model_name="market-segmentation", model_version=_model_version, model_type="clustering")
        _reference_data = _load_reference_data()
        logger.info("Model reloaded dynamically", model="market-segmentation", version=_model_version)
