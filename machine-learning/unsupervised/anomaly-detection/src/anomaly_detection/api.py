...
from mlops_shared.validation import DataValidator, create_anomaly_detection_schema
...
async def lifespan(app: FastAPI):
    ...
    _metrics.set_model_info(model_name="anomaly-detection", model_version=_model_version, model_type="anomaly_detection")
...
def _load_model() -> tuple[PCAAnomalyDetector, str]:
    ...
    try:
        if MODEL_VERSION == "latest":
            ...
            if ad_models:
                ...
                npz_files = list(model_dir.glob("anomaly_detection_model_*.npz")) + list(model_dir.glob("*.npz"))
                if npz_files:
    ...
    candidate_paths = [
        ...
        Path(__file__).resolve().parents[3] / "artifacts" / "models" / "anomaly_detection_model_v1.0.0.npz",
    ]
...
def get_stats():
    ...
    return StatsResponse(
        ...
        mean_reconstruction_error=float(
            np.mean(_model.reconstruction_error(_reference_data)) if _reference_data is not None else 0.0
        ),
        max_reconstruction_error=float(
            np.max(_model.reconstruction_error(_reference_data)) if _reference_data is not None else 0.0
        ),
        model_version=_model_version,
    )
