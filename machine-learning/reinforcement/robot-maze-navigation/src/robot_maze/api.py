...
async def lifespan(app: FastAPI):
    ...
    _metrics.set_model_info(model_name="robot-maze-navigation", model_version=_model_version, model_type="reinforcement_learning")
...
def _load_model() -> tuple[QLearningAgent, str]:
    ...
    try:
        if MODEL_VERSION == "latest":
            ...
            if rm_models:
                ...
                npz_files = list(model_dir.glob("robot_maze_model_*.npz")) + list(model_dir.glob("*.npz"))
                if npz_files:
    ...
    candidate_paths = [
        ...
        Path(__file__).resolve().parents[3] / "artifacts" / "models" / "robot_maze_model_v1.0.0.npz",
    ]
...
def reload_model():
    ...
    try:
        ...
        if _metrics:
            ...
            _metrics.set_model_info(model_name="robot-maze-navigation", model_version=_model_version, model_type="reinforcement_learning")
        _reference_data = _load_reference_data()
        logger.info("Model reloaded dynamically", model="robot-maze-navigation", version=_model_version)
...
def get_stats():
    ...
    return StatsResponse(
        ...
        mean_training_error=float(np.mean(_model.training_errors)) if _model.training_errors else 0.0,
        mean_episode_reward=float(np.mean(_model.episode_rewards)) if _model.episode_rewards else 0.0,
        mean_episode_length=float(np.mean(_model.episode_lengths)) if _model.episode_lengths else 0.0,
        model_version=_model_version,
    )
...
def trigger_training(body: TrainRequest):
    ...
    try:
        ...
        if body.mode == "online":
            ...
            from robot_maze.data import load_training_data
