...
def train(
    ...
    if mode == "online":
        ...
        save_training_data(states, actions, rewards, next_states, dones, model_dir / "training_data.npz")
    ...
    registry.save_model(
        ...
        tags={"framework": "numpy", "task": "reinforcement_learning", "method": "q_learning", "mode": mode},
    )
    ...
    if register_to_mlflow:
        registry.log_to_mlflow(
            ...
            tags={"model_type": "reinforcement_learning", "framework": "numpy", "method": "q_learning", "mode": mode},
        )
        logger.info("Registered model to MLflow", model="robot-maze-navigation", version=model_version)
...
def _save_chart(agent: QLearningAgent, maze: np.ndarray, output_dir: Path, version: str) -> None:
    ...
    if agent.episode_rewards:
        ...
        if window > 0:
            ...
            ax2.plot(range(window - 1, len(agent.episode_rewards)), moving_avg, color="red", label=f"MA({window})")
...
def main():
    ...
    parser.add_argument("--learning-rate", type=float, default=float(os.getenv("LEARNING_RATE", "0.1")))
    parser.add_argument("--discount-factor", type=float, default=float(os.getenv("DISCOUNT_FACTOR", "0.99")))
    parser.add_argument("--epsilon-decay", type=float, default=float(os.getenv("EPSILON_DECAY", "0.995")))
    parser.add_argument("--mode", type=str, default=os.getenv("RL_MODE", "online"), choices=["online", "offline"])
