...
def train(
    ...
    if test_metrics["accuracy"] < 0.8:
        logger.warning("Model accuracy below threshold", accuracy=test_metrics["accuracy"], threshold=0.8)
    ...
    registry.save_model(
        ...
        metrics=metrics,
        parameters={"learning_rate": learning_rate, "n_iterations": n_iterations, "random_seed": random_seed},
        artifacts={
    ...
    if register_to_mlflow:
        registry.log_to_mlflow(
            ...
            model_version=model_version,
            metrics=metrics,
            params={"learning_rate": learning_rate, "n_iterations": n_iterations, "random_seed": random_seed},
            artifacts={
        ...
        logger.info("Registered model to MLflow", model="spam-classification", version=model_version)

    return metrics


def _save_chart(model: LogisticRegression, X: np.ndarray, y: np.ndarray, output_dir: Path, version: str) -> None:
